...
class GA:
    def __init__(self, listGAParameters, fp_obInstance, fp_local_state):
        ...
        self.iSceneNum = fp_obInstance.iScenNum
        if self.obInstance.iSitesNum != self.iIndLen:
            print(
                "Wrong. The number of candidate sites is not equal to the individual length. Please check.", flush=True
            )
    ...
    def funEvaluateInd(self, fp_aChromosome):
        ...
        if fp_aChromosome.size != self.obInstance.aiFixedCost.size:
            print("Wrong. Please make sure that the size of variable \"fp_aChromosome\" and \"self.obInstance.aiFixedCost\" equal.", flush=True)
        w1 += np.dot(fp_aChromosome, self.obInstance.aiFixedCost)
        iSelcSitesNum = np.sum(fp_aChromosome)
        if iSelcSitesNum == 0:
            return 0

        for s in range(self.iSceneNum):
            for i in range(self.iIndLen):  # i represents different customers.
                ...
                if iSelcSitesNum != len(aSelcSitesTransCostForI):
                    print("Wrong in funEvaluatedInd(). Please check.", flush=True)
                ...
                for j in range(iAlloFaciNum):
                    ...
                    q = self.obInstance.fScenProb
                    w2 += q * self.obInstance.listaiDemands[s][i] * aSortedTransCostForI[
                        j] * pow(p, j) * (1 - p)
    ...
    def funCrossover(self,
        ...
        if len(fp_listdictCurrPop) != self.iPopSize:
            print(
                "Someting wrong. The population size before crossover is abnormal.", flush=True
            )
    ...
    def funLocalNeighborhood(self, fp_listdictCurrPop):
        ...
        print("搜索过邻域的个体数:", len(self.listaLocalSearchTestRepeat), flush=True)
    ...
    def funWeakLocalNeighborhood(self, fp_listdictCurrPop):
        '''
        Use local search only to the best 10 individuals;
        如果已经被局部搜索过，则不再对该个体进行局部搜索。
        '''
        print('Weak', flush=True)
        listdictNeighborPop = []
    ...
    def funWeakLocalNeighborhood2(self, fp_listdictCurrPop):
        '''
        Use local search to fp_listdictCurrPop.
        不只局限于前10个个体。从第一个开始依次往后，跳过已经局部搜索过的个体，直到达到self.iLocalSearchIndNumEveGen所规定的数目。
        '''
        print('Weak-2', flush=True)
        ...
        print("局部搜索过的个体数量:", self.listiEveGenLocalSearchedIndNum[-1], flush=True)
        listdictNeighborPopAfEva = self.funEvaluatePop(listdictNeighborPop)
    ...
    def funGenerateNeighbor(self, fp_aChromosome):
        if self.iIndLen != fp_aChromosome.size:
            print("Wrong in funGetIndNeighbor()", flush=True)
        listaNeighbor = []
    ...
    def funGA_main(self):
        ...
        for gen in range(self.iGenNum):
            ...
            print("self.iPopSize:", self.iPopSize, flush=True)
            print("Actual PopSize:", len(listdictCurrPop), flush=True)
        ...
        for i in range(len(listdictInitPop)):
            for j in range(len(self.listaLocalSearchTestRepeat)):
                ...
                if iHammingDist <= 1:
                    ...
                    break
        print("iInitIndInSearchedNeighborNum:", iInitIndInSearchedNeighborNum, flush=True)
...
if __name__ == '__main__':
    ...
    listInstPara = [iCandidateFaciNum, iScenNum, iDemandLB, iDemandUB, iFixedCostLB, iFixedCostUB, iCoordinateLB, iCoordinateUB, fFaciFailProb, fScenProb]

    start = time.process_time()
    # generate instance
    obInstance = instanceGenerationRRFLP.Instances(listInstPara)
    ...
    for f in range(len(listfFeIndex)):
        listFeXCoordinate.append(listiFitEvaNumByThisGen[int(listfFeIndex[f])])
    ...
    print("CPU Time:", end - start, flush=True)
